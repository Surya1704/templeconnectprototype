...
import { TOTAL_TEMPLE_COUNT } from "@/data/temples";
import { ArrowRight } from "lucide-react";

const TRIMBAKESHWAR_DONATION_URL = "https://trimbakeshwartrust.com/donate";
...
function TrustStrip() {
  return (<SectionReveal><section className="bg-bg-secondary border-t border-b border-line-hair"><div className="max-w-container mx-auto px-5 md:px-8 h-[120px] flex items-center justify-center"><div className="grid grid-cols-2 w-full max-w-[640px] gap-4 md:gap-0 md:divide-x md:divide-line-hair"><NumberStat value={`${TOTAL_TEMPLE_COUNT}+`} label="temples on the platform" /><NumberStat value="100%" label="goes to temples" italicLabel /></div></div></section></SectionReveal>);
}
...
function DonationMoment() {
  return (<SectionReveal><section className="bg-bg-primary py-16 md:py-24"><div className="max-w-container mx-auto px-5 md:px-8"><div className="flex flex-col md:flex-row items-stretch gap-0"><div className="w-full md:w-1/2 relative"><img src="/jyotirlingas/trimbakeshwar.png" alt="Trimbakeshwar Temple" className="w-full h-full object-cover rounded-r-[16px] rounded-l-none min-h-[300px] md:min-h-[480px]" /></div><div className="w-full md:w-1/2 py-8 md:py-12 md:pl-12 flex flex-col justify-center"><Eyebrow>ONE TEMPLE, THIS MONTH</Eyebrow><h2 className="mt-4 font-serif text-[36px] md:text-[44px] font-normal leading-[1.1] tracking-[-0.018em] text-ink-primary">Help Trimbakeshwar restore <em>its 17th-century inner sanctum</em>.</h2><p className="mt-4 font-serif text-[17px] text-ink-secondary leading-[1.65] max-w-[480px]">The Trimbakeshwar Devasthan Trust runs conservation and seva work at the temple. Contributions are collected and accounted for entirely by the official trust — FaithConnect never holds or processes your donation.</p><p className="mt-3 font-sans text-[13px] text-ink-tertiary leading-[1.6]">Donations go directly to the Trimbakeshwar Devasthan Trust&apos;s official portal.</p><div className="mt-6 flex flex-col items-start gap-3"><a href={TRIMBAKESHWAR_DONATION_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center px-7 py-3.5 bg-accent text-bg-card font-sans text-[14px] font-medium rounded-pill hover:bg-accent-deep hover:-translate-y-0.5 transition-all duration-320 fc-out">Donate via the official trust</a><a href="https://trimbakeshwartrust.com/" target="_blank" rel="noopener noreferrer" className="font-sans text-[13px] text-ink-secondary hover:underline underline-offset-2 transition-colors duration-160">Visit the trust website →</a></div></div></div></div></section></SectionReveal>);
}
...
export default function Home() { return (<><Hero /><TrustStrip /><JyotirlingaSection /><WhatWeDo /><DonationMoment /><ForTempleBoards /></>); }
